...
//                     (player == 1) ? (score += 900000) : (score += 90);
//         }
//     }
...
//         maxMoves = max(maxMoves, tempMoves);
//     }
//     (player == 1) ? (score += (maxMoves*10000)) : (score = (maxMoves*10));
// // ***************************************************************
// // ************************ end Feature 3 win ********************
// // ***************************************************************
...
//     if(column == 3)
//         (player == 1) ? (score += 2000) : (score += 20);
//     else if(column == 0 || column == 6)
//         (player == 1) ? (score += 400) : (score += 4);
//     else if(column == 1 || column == 5)
//         (player == 1) ? (score += 700) : (score +=7);
//     else
//         (player == 1) ? (score += 1200) : (score += 12);

//     (player == 1) ? (score += scoreMat[row - 3][column]*10) : (score += scoreMat[row - 3][column]);
...
int heuristic12(long long state, long long pastState, char player)
{
    // player = 1 ==> green
    // player = 2 ==> red
    char opponent = (player == 1) ? 1 : 2;
...
long long minimaxAB(long long state, long long pastState, int depth, long long alpha, long long beta, bool maximizingPlayer, int emptySlots)
{
    ...
    if (depth == 0 || emptySlots == 0) // or game is over in position
    {
        ...
        // ((oriDepth % 2) == 0 )? (score = -1*score) : (score = score);
    ...
    if (maximizingPlayer)
    {
        ...
        for(auto neighbor : neighbors)
        {
            ...
            // long long tempScore = heuristic(neighbor, state, player);
            // long long tt = tempScore + eval.first;
    ...
    else
    {
        ...
        for(auto neighbor : neighbors)
        {
            ...
            // eval = minimaxAB(neighbor, state, depth - 1, alpha, beta, true, emptySlots - 1);
            // // eval = -1*eval;
            // // long long tempScore = -1*heuristic(neighbor, state, player);
            // // long long tt = tempScore + eval.first;
            // if(eval <= minEval)
            // {
            //     chosenAction = neighbor;
...
pair<long long,long long> minimaxAB12(long long state, long long pastState, int depth, long long alpha, long long beta, bool maximizingPlayer, int emptySlots)
{
    char player = maximizingPlayer ? 2 : 1;
    if (depth == 0 || emptySlots == 0) // or game is over in position
    {
    ...
    if (maximizingPlayer)
    {
        ...
        for(auto neighbor : neighbors)
        {
            eval = minimaxAB12(neighbor, state, depth - 1, alpha, beta, false, emptySlots - 1);
    ...
    else
    {
        ...
        for(auto neighbor : neighbors)
        {
            eval = minimaxAB12(neighbor, state, depth - 1, alpha, beta, true, emptySlots - 1);
            // eval.first = -1*eval.first; 
            if(eval.first <= minEval.first)
                chosenAction = neighbor;
